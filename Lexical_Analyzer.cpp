...
bool validKeyword(char* str) {
    const char* keywords[] = {"if", "else", "while", "do", "break", "continue", "int", "double", "float", "return", "char", "case", "long", "short", "typedef", "switch", "unsigned", "void", "static", "struct", "sizeof", "volatile", "enum", "const", "union", "extern", "bool", "#include", "using", "namespace", "std", "<iostream>", "main"};
...
void parse(char* str) {
    ...
    int len = strlen(str);
    while (right <= len) {
        if (isspace(str[right]) || !validPunctuator(str[right])) {
            right++;
            continue;
        }
